...
func create_map():
	...
	while y >= 2:
		...
		#var yhallways: int = rng.randi_range(0, 1) + int(size / 6)
	...
	var room1_count: int = 0
	var room2_count: int = 0
	var room2c_count: int = 0
	var room3_count: int = 0
	var room4_count: int = 0
	#spawn a room
	for n in range(size):
		for o in range(size):
			...
			match temp_room[n][o].type:
				RoomTypes.ROOM1:
					if (room1_count >= rooms.endrooms_single.size()):
						selected_room = rooms.endrooms[rng.randi_range(0, rooms.endrooms.size() - 1)]
					else:
						selected_room = rooms.endrooms_single[room1_count]
					room1_count += 1
					...
					add_child(room, true)
				RoomTypes.ROOM2:
					if (room2_count >= rooms.hallways_single.size()):
						selected_room = rooms.hallways[rng.randi_range(0, rooms.hallways.size() - 1)]
					else:
						selected_room = rooms.hallways_single[room2_count]
					room2_count += 1
					room = selected_room.instantiate()
					room.position = Vector3(n * grid_size, 0, o * grid_size)
					room.rotation_degrees = Vector3(0, temp_room[n][o].angle, 0)
					add_child(room, true)
				RoomTypes.ROOM2C:
					if (room2c_count >= rooms.corners_single.size()):
						selected_room = rooms.corners[rng.randi_range(0, rooms.corners.size() - 1)]
					else:
						selected_room = rooms.corners_single[room2c_count]
					room2c_count += 1
					room = selected_room.instantiate()
					room.position = Vector3(n * grid_size, 0, o * grid_size)
					room.rotation_degrees = Vector3(0, temp_room[n][o].angle, 0)
					add_child(room, true)
				RoomTypes.ROOM3:
					if (room3_count >= rooms.trooms_single.size()):
						selected_room = rooms.trooms[rng.randi_range(0, rooms.trooms.size() - 1)]
					else:
						selected_room = rooms.trooms_single[room3_count]
					room3_count += 1
					room = selected_room.instantiate()
					room.position = Vector3(n * grid_size, 0, o * grid_size)
					room.rotation_degrees = Vector3(0, temp_room[n][o].angle, 0)
					add_child(room, true)
				RoomTypes.ROOM4:
					if (room4_count >= rooms.crossrooms_single.size()):
						selected_room = rooms.crossrooms[rng.randi_range(0, rooms.crossrooms.size() - 1)]
					else:
						selected_room = rooms.crossrooms_single[room4_count]
					room4_count += 1
					room = selected_room.instantiate()
					room.position = Vector3(n * grid_size, 0, o * grid_size)
					room.rotation_degrees = Vector3(0, temp_room[n][o].angle, 0)
